...
- **Lessons** — **platform lessons** and operating insights, not just the local fix.
...
- [`slow-join-skew-case-study.md`](slow-join-skew-case-study.md) — A batch fact-to-dimension join whose runtime spiked when one join key “went viral.” The story is about long-tail tasks in a sort-merge join, profiling keys before you tune `spark.sql.shuffle.partitions`, and AQE / salting with guardrails.
...
The intended audience is a production engineer who has seen Spark in production and wants the diagnostic loop, the smallest fix, and the platform lessons — not a tutorial. If you are new to Spark, read the relevant book chapter first and come back here.
...
- Working on a slow **join** with a long task tail: `slow-join-skew-case-study.md` first.
